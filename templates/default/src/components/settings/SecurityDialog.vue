  ...
  <div class="flex flex-col gap-5">
    <GlassCard spotlight class="p-4 sm:p-6">
      <div class="flex items-center gap-3">
        <span class="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-emerald-400/20 to-teal-500/20 text-emerald-200 ring-1 ring-edge">
        ...
        <div>
          <h2 class="text-base font-semibold text-ink">{{ t("security.changePassword.title") }}</h2>
          <p class="mt-0.5 text-sm text-mute">{{ t("security.changePassword.hint") }}</p>
        </div>
      </div>
    ...
    <GlassCard spotlight class="p-4 sm:p-6">
      <div class="flex items-center gap-3">
        <span class="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-amber-400/20 to-orange-500/20 text-amber-200 ring-1 ring-edge">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4">
        ...
        <div>
          <h2 class="text-base font-semibold text-ink">{{ t("security.protection.title") }}</h2>
          <p class="mt-0.5 text-sm text-mute">{{ t("security.protection.hint") }}</p>
        </div>
      </div>

      <div class="mt-5 flex flex-col divide-y divide-edge">
        <div class="flex flex-wrap items-center justify-between gap-3 py-4 first:pt-0">
          <div>
            <p class="text-sm font-medium text-ink2">{{ t("security.twoFactor.title") }}</p>
            <p class="mt-0.5 text-xs text-mute">{{ t("security.twoFactor.hint") }}</p>
          </div>
          <div class="flex items-center gap-3">
            <span
        ...
        <div class="py-4">
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p class="text-sm font-medium text-ink2">{{ t("security.sessions.title") }}</p>
              <p class="mt-0.5 text-xs text-mute">{{ t("security.sessions.hint") }}</p>
            </div>
          </div>
          <ul class="mt-4 flex flex-col gap-2.5">
            <li
              v-for="s in sessions"
              :key="s.name"
              class="flex items-center gap-3 rounded-xl border border-edge bg-glass px-3.5 py-3"
            >
              <span class="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-glass text-mute ring-1 ring-edge">
              ...
              <div class="flex-1">
                <p class="text-sm font-medium text-ink2">
                  {{ s.name }}
                ...
                <p class="mt-0.5 text-xs text-mute">{{ s.meta }}</p>
              </div>
              <button v-if="!s.current" class="text-xs text-mute transition hover:text-rose-300">
